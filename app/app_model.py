...
def predict_crop():
    try:
        ...
        for key in required_keys:
            if key not in data:
                return jsonify({"error": f"Missing data for key: {key}"}), 400
            # Ensure all values are float
            data[key] = float(data[key])
        ...
        app.logger.info(f"Input features: {input_features}")
        ...
        app.logger.info(f"Predicted class: {predicted_class}, Predicted crop: {predicted_crop}")
